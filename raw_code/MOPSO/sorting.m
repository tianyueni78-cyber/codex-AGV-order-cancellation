...
OS_REAL = chrom(1: operaNum);
MS_REAL = chrom(operaNum + 1: 2 * operaNum);
AS_REAL = chrom(2 * operaNum + 1 : 3 * operaNum);
SS_REAL = chrom(3 * operaNum + 1: 5 * operaNum);
...
[~, upIndex] = sort(OS_REAL);
[~, osIndex] = sort(upIndex);%对应原始chrom中的111122223333444顺序
OS = OS(osIndex); % 将小数OS转成了fjsp中常用的整数OS
%% 将以上染色体片段 转为整数
% OS
OS = [];
for i = 1: jobNum
...
% MS AS SS
MS = zeros(1, size(MS_REAL, 1));
AS = zeros(1, size(AS_REAL, 1));
SS = zeros(1, size(SS_REAL, 1));
cnt = 0;
speedNum = length(AGVSpeed);    % AGV速度挡位数目
for i = 1: jobNum
    for j = 1: operaVec(i)
        cnt = cnt + 1;
        MS(cnt) = round(1 / (2 * jobNum) * (MS_REAL(cnt) + jobNum) * (length(candidateMachine{i,j}) - 1) + 1);
        AS(cnt) = round(1 / (2 * jobNum) * (AS_REAL(cnt) + jobNum) * (AGVNum - 1) + 1);
        SS(2 * cnt - 1) = round(1 / (2 * jobNum) * (SS_REAL(2 * cnt - 1) + jobNum) * (speedNum - 1) + 1);
        SS(2 * cnt) = round(1 / (2 * jobNum) * (SS_REAL(2 * cnt) + jobNum) * (speedNum - 1) + 1);
    end
end
