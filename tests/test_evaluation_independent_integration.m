...
[problem, machineData, agvData, config, chrom] = make_independent_case();
...
result = evaluate_decoded_schedule( ...
    decodedResult, problem, machineData, agvData, config);

assert(isfinite(result.makespan), 'makespan should be finite.');
assert(isfinite(result.machineEnergy), 'machineEnergy should be finite.');
assert(isfinite(result.agvEnergy), 'agvEnergy should be finite.');
assert(isfinite(result.totalEnergy), 'totalEnergy should be finite.');
assert(isequal(result.objectives, [result.makespan, result.totalEnergy]), ...
    'objectives should be [makespan, totalEnergy].');

fprintf('test_evaluation_independent_integration passed: makespan=%.6f, totalEnergy=%.6f\n', ...
    result.makespan, result.totalEnergy);

function [problem, machineData, agvData, config, chrom] = make_independent_case()
problem.jobNum = 2;
