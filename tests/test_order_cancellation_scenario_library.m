...
assert(all(cancel_job_ids(shortRows) == 1), ...
    'short category should select the fewest-operation job.');
assert(all(cancel_job_ids(longRows) == 2), ...
    'long category should select the most-operation job.');
assert(all(cancel_job_ids(criticalRows) == 2), ...
    'critical category should select the latest-completing job.');
assert(all(cancel_job_ids(noncriticalRows) == 3), ...
    'noncritical category should select the earliest-completing job.');
end

function jobIds = cancel_job_ids(rows)
jobIds = arrayfun(@(row) row.cancel.job_id, rows);
end
