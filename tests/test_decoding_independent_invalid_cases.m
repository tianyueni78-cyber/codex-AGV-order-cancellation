...
[problem, machineData, agvData, config, validChrom] = make_independent_decoding_case();

chrom = validChrom(1:end - 1);
expect_invalid(chrom, problem, machineData, agvData, config, ...
...
expect_invalid(validChrom, problem, missingMachineData, agvData, config, ...
    'missing-required-fields', 'machineData.distance_matrix is required');

try
    decode_population_independent([], problem, machineData, agvData, config);
    error('test_decoding_independent_invalid_cases:ExpectedError', ...
        'decode_population_independent should reject an empty population.');
catch err
    assert(~isempty(strfind(err.identifier, ...
        'decode_population_independent:InvalidPopulation')), ...
        'Empty population should raise decode_population_independent:InvalidPopulation.');
end

fprintf('test_decoding_independent_invalid_cases passed\n');

function expect_invalid(chrom, problem, machineData, agvData, config, ...
    expectedStatus, expectedMessage)
[~, report] = decode_chromosome_independent(chrom, problem, machineData, agvData, config);
assert(~report.isValid, 'Expected independent decoding to be invalid.');
