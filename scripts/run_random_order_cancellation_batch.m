...
for i = 1:numel(rows)
    ...
    fprintf(fid, ['%s,%d,%.6f,%g,%s,%s,%d,%d,%.6f,%.6f,%.6f,%.6f,', ...
        '%s,%s,%s,%s,%s,%s,%s,%d,%d,%s,%d,%d,%d,%d,%d,%s,%s\n'], ...
        ...
        csv_text(row.complete_rescheduling_validation_probe), ...
        csv_text(row.complete_rescheduling_rejected_reason), ...
        row.complete_rescheduling_error_count, ...
        row.complete_rescheduling_rejected_count, ...
...
row.complete_rescheduling_validation_probe = '';
row.complete_rescheduling_rejected_reason = '';
row.complete_rescheduling_error_count = 0;
row.complete_rescheduling_rejected_count = 0;
...
[row.complete_rescheduling_validation_probe, ...
    ...
    row.complete_rescheduling_error_count, ...
    row.complete_rescheduling_rejected_count] = ...
    build_complete_rescheduling_diagnostics(completeCandidate, ...
    completeEvaluation, selection, state, cancel, ...
...
if isstruct(selection) && isfield(selection, 'candidates') && ...
    ...
    if isfield(selection.candidates, selectionFieldName)
        ...
        parts{end + 1} = ['selection_', selectionFieldName, ...
            '_rejectedReasons_full=', ...
...
function [validationProbe, rejectedReason, errorCount, rejectedCount] = ...
    build_complete_rescheduling_diagnostics(candidate, evaluation, ...
    selection, state, cancel, fallbackErrorText)
validationProbe = build_validation_probe(candidate, evaluation, selection, ...
    'complete_rescheduling');
[rejectedReason, rejectedCount, errorCount] = ...
    summarize_complete_rescheduling_report(candidate, evaluation, selection);

if isempty(rejectedReason)
    report = struct();
    if isstruct(candidate) && isfield(candidate, 'report') && ...
            isstruct(candidate.report)
        report = candidate.report;
    elseif isstruct(evaluation) && isfield(evaluation, 'report') && ...
            isstruct(evaluation.report)
        report = evaluation.report;
    elseif isstruct(selection) && isfield(selection, 'report') && ...
            isstruct(selection.report)
        report = selection.report;
    end
    rejectedReason = summarize_candidate_report_diagnostics(report);
end

if isempty(rejectedReason) && ~isempty(fallbackErrorText)
    rejectedReason = char(string(fallbackErrorText));
end
...
function [rejectedReason, rejectedCount, errorCount] = ...
    summarize_complete_rescheduling_report(candidate, evaluation, selection)
rejectedReason = '';
rejectedCount = 0;
errorCount = 0;
...
if is_nonempty_cell_field(report, 'rejectedReasons')
    rejectedReason = summarize_first_text(report.rejectedReasons);
    rejectedCount = numel(report.rejectedReasons);
end
if is_nonempty_cell_field(report, 'errors')
    if isempty(rejectedReason)
        rejectedReason = summarize_first_text(report.errors);
    end
    errorCount = numel(report.errors);
end
if isempty(rejectedReason) && is_nonempty_cell_field(report, 'warnings')
    rejectedReason = summarize_first_text(report.warnings);
end

if isempty(rejectedReason) && isstruct(evaluation) && isfield(evaluation, 'report') && ...
        isstruct(evaluation.report)
    if isfield(evaluation.report, 'rejectedReasons') && ...
            iscell(evaluation.report.rejectedReasons) && ...
            ~isempty(evaluation.report.rejectedReasons)
        rejectedReason = summarize_first_text(evaluation.report.rejectedReasons);
    elseif isfield(evaluation.report, 'errors') && ...
            iscell(evaluation.report.errors) && ...
            ~isempty(evaluation.report.errors)
        rejectedReason = summarize_first_text(evaluation.report.errors);
    elseif isfield(evaluation.report, 'warnings') && ...
            iscell(evaluation.report.warnings) && ...
            ~isempty(evaluation.report.warnings)
        rejectedReason = summarize_first_text(evaluation.report.warnings);
    end
end

if isempty(rejectedReason) && isstruct(selection) && isfield(selection, 'report') && ...
        isstruct(selection.report) && isfield(selection.report, 'errors') && ...
        iscell(selection.report.errors) && ~isempty(selection.report.errors)
    rejectedReason = summarize_first_text(selection.report.errors);
end
end
