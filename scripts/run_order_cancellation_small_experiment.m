...
for i = 1:numel(scenarioRows)
    ...
    fprintf(fid, ['%s,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,', ...
        '%.6f,%.6f,%.6f,%.6f,%d,%d\n'], ...
        row.scenario_name, row.run_count, ...
        row.local_feasible_count, ...
        row.complete_feasible_count, ...
        row.local_Cmax_delta_mean, ...
        row.complete_Cmax_delta_mean, ...
        row.local_SD_mean, ...
        row.complete_SD_mean, ...
        row.local_TD_mean, ...
        row.complete_TD_mean, ...
        row.local_energy_delta_mean, ...
        row.complete_energy_delta_mean, ...
        row.local_Y_mean, ...
        row.complete_Y_mean, ...
...
summary = summarize_order_cancellation_results(results);
strategyRows = summary.strategy_counts;
fid = fopen(filePath, 'w');
if fid < 0
    error('small_experiment:FileOpenFailed', ...
...
for i = 1:numel(strategyRows)
    row = strategyRows(i);
    fprintf(fid, '%s,%d\n', row.selected_strategy, row.count);
end
end
