...
config.hybrid_policy = build_hybrid_policy_config();
...
decision = select_hybrid_cancellation_policy( ...
    ...
    completeEvaluation, config);

fprintf('order cancellation hybrid policy smoke\n');
...
fprintf('decision.isSelected: %d\n', decision.isSelected);
fprintf('decision.selected_strategy: %s\n', decision.selected_strategy);
fprintf('decision.reason: %s\n', decision.reason);
fprintf('decision.triggered_complete_rescheduling: %d\n', ...
    decision.triggered_complete_rescheduling);
fprintf('decision.threshold_report.cmax_delta_triggered: %d\n', ...
    decision.threshold_report.cmax_delta_triggered);
fprintf('decision.threshold_report.energy_delta_triggered: %d\n', ...
    decision.threshold_report.energy_delta_triggered);
fprintf('decision.threshold_report.idle_waste_triggered: %d\n', ...
    decision.threshold_report.idle_waste_triggered);
fprintf('error_count: %d\n', numel(decision.report.errors));
fprintf('rejected_reason_count: %d\n', ...
    numel(decision.report.rejectedReasons));
...
function hybridPolicy = build_hybrid_policy_config()
hybridPolicy = struct();
hybridPolicy.enable_complete_if_local_infeasible = true;
hybridPolicy.use_stage_e_y_selection = true;
hybridPolicy.cmax_delta_threshold = 0;
hybridPolicy.energy_delta_threshold = 0;
hybridPolicy.idle_waste_threshold = Inf;
hybridPolicy.threshold_validation_status = ...
...
for i = 1:operaNum
    ...
    if isempty(candidateMachines)
        error('hybrid_policy_smoke:MissingCandidateMachine', ...
